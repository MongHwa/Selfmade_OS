...
extern "C" void KernelMainNewStack(const FrameBufferConfig& frame_buffer_config_ref,
    ...
    /* 메모리 관리 관련 사항 정리*/
    /*
        1. UEFI BIOS 측이 갖고 있는 메모리 맵을 Kernel에서 넘겨받음
        2. 그 메모리 맵을 참조해 메모리를 관리하기 전에 준비 필요
            -> 몇 가지 데이터 구조를 OS가 소유한 메모리 영역으로 이동해야함.
        3. (1) 스택 영역
           (2) 세그먼테이션 설정 영역 (GDT)
           (3) 페이지 테이블
        
        4. 스택 영역의 이동 == RSP 값 고쳐쓰기
        5. 세그먼테이션 설정 -> (1) GDT 재구축 / (2) GDT를 CPU에 반영 / (3) 페이지 테이블 설정으로 이동
        6. 페이지 테이블 설정 (페이지 맵 4계층 구조를 기억하자!)
            (1) 페이지 맵 레벨4 테이블 (PML4 table)
            (2) 페이지 디렉터리 포인터 테이블 (PDP table)
            (3) 페이지 디렉터리
            (4) 페이지 테이블
        7. 메모리 할당 및 해제 기능 구현 (memory_manager.hpp, memory_manager.cpp 참고)
    */
